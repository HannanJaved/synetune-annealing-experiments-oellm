...
OUTPUT=$(python sample_data_weights_constrained.py 2>&1)

# Parse the output and write to file
echo "$OUTPUT" | awk '/New data weights \(sum to 100\):/ {flag=1; next} flag && /^\/.*: [0-9]/ {split($0, a, ": "); print a[2], a[1]}' > ${BASE_PATH}/run18/data_paths.txt
