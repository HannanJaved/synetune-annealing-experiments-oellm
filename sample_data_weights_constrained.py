...
def main():
    ...
    # Enforce max constraints for finemath and starcoder
    if new_config is not None:
        # Apply max constraints
        if new_config['finemath_weight'] > 20: # Math data should not exceed 20%
            new_config['finemath_weight'] = 20
        if new_config['starcoder_weight'] > 25: # Code data should not exceed 25%
            new_config['starcoder_weight'] = 25
        # Renormalize the rest to sum to 100
        total = sum(new_config.values())
        if total > 100:
            # Scale down all weights proportionally to sum to 100
            scale = 100 / total
            new_config = {k: v * scale for k, v in new_config.items()}
        else:
            # If total < 100, scale up the remaining group(s) (nemotron) to fill to 100
            remainder = 100 - (new_config['finemath_weight'] + new_config['starcoder_weight'])
            new_config['nemotron_weight'] = remainder
